...
export default function LapsTimeReport() {
  ...
  // Estados para heurísticas
  const [isLoading, setIsLoading]     = useState(false);
  const [formError, setFormError]     = useState('');
  const [hasSearched, setHasSearched] = useState(false); // ⬅️ nuevo

  const handleGenerate = async e => {
    e.preventDefault();
    setHasSearched(true); // ⬅️ marcamos que se intentó buscar

    // Validación inline (Prevención de errores)
  ...
  const handleReset = () => {
    ...
    setReports([]);
    setFormError('');
    setHasSearched(false); // ⬅️ limpiamos la búsqueda
  };
  ...
  // construye el mapa cat → mes → ingreso
  const dataMap = {};
  reports.forEach(r => {
  ...
  return (
    <div className="reports-container">
      <h2>Reporte de ingresos por número de vueltas o tiempo máximo</h2>
      ...
      {/* ⬅️ Mensaje cuando no hay resultados tras buscar */}
